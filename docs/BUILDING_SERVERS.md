...
4. **Map arguments to a `Task`.** `goal` describes the exact behavior and observable completion conditions. `values` maps semantic labels to all exact strings that may need typing. Jev cannot generate missing text: include computed search strings, dates formatted for the UI and every line-item field. Known calculations/formatting belong in code. `uploads` maps attachment purposes to existing file-store names. `start_url` is only for a site-computed, domain-permitted URL derived from validated arguments (such as a numeric record ID), never a caller-supplied URL. Set a bounded `max_steps`.

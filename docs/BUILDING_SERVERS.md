...
7. **Run with `website-mcp serve --site servers/<site>/site.py`.** `--site` takes a file path or an importable module; either must export `SITE`. No registry edits. `website_task` and the file tools are added automatically.
8. **Package and isolate.** Copy `servers/wikipedia/compose.yaml` to `servers/<site>/compose.yaml`: set `name`, service `<site>-mcp`, `SITE_DIR: servers/<site>`, a free host port, and volume name `<site>-mcp-data`. The generic `Dockerfile` copies the site directory to `/site`. For multiple accounts, use different `--account` values, ideally separate containers/volumes. Never mount a real desktop browser profile. Set `login_domains` so the runner reports `login_required` with the in-process login view URL instead of spending model calls on a sign-in page.

...
async def test_http_transport_serves_site_tools(tmp_path, token):
    with socket.socket() as probe:
        probe.bind(("127.0.0.1", 0))
        port = probe.getsockname()[1]
    env = {k: v for k, v in os.environ.items() if k != "WEBSITE_MCP_DEBUG_TOOLS"}
    env["WEBSITE_MCP_TOKEN"] = token
    ...
    try:
        async with httpx.AsyncClient(base_url=base) as client:
            ...
            assert (await client.get("/screen")).status_code == 401  # login view always guarded
            if token:

...
Status on 2026-09-17: no authenticated mailbox has been exercised yet. The container starts, lists 16 tools, reaches the public Outlook landing page and reports `login.required: true` (`scripts/check_docker.py servers/outlook`). Argument validation and search syntax are covered by `tests/test_outlook.py`.
...
| Attachments | `outlook_download_attachments` + `outlook_read_file` | download primitive (fixture) | Real attachment bytes and filename |
